...
public:
   ...
   mart(eosio::name receiver, eosio::name code, datastream<const char*> ds):
   ...
   /**
    *  This action issues to `to` account a `quantity` of marts.
    *
    * @param to - the account to issue marts to, it must be the same as the issuer,
    * @param quntity - the amount of marts to be issued,
    * @memo - the memo string that accompanies the mart issue transaction.
    */
   [[eosio::action]]
   void issue( const name& to, const mart_asset& quantity, const string& memo );

   [[eosio::action]]
   void retire( const mart_asset& quantity, const string& memo );
    ...
    */
   [[eosio::action]]
   ACTION transfer(const name& from, const name& to, const mart_asset& quantity, const string& memo );
   using transfer_action = action_wrapper< "transfer"_n, &mart::transfer >;

   ACTION setpowasset( const name& issuer, const uint64_t symbid, const pow_asset_meta& asset_meta);

   private:
   void add_balance( const name& owner, const mart_asset& value );
   void sub_balance( const name& owner, const mart_asset& value );
};
} //namespace apollo
