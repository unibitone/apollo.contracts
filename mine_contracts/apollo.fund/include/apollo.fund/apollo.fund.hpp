...
class [[eosio::contract("apollo.mart")]] mart : public eosio::contract
{
    ...
    public:
        ...
        [[eosio::action]] void updatetoken(const uint64_t& id,const asset& electricity_fee,const asset& token_price,const name& status,const name& sell_type,const uint8_t& discount);
        
        [[eosio::action]] void updatestatus(const uint64_t& id,const name& status);
